...
# Attempt to diagnose problems with the dlmat server that the server itself
# can't diagnose (since it might not even be alive in the first place, lacking
# CGI and so on).
#
# Example:
#
#    tools/troubleshooting.sh http://localhost/~dlmat/
#
...
if [ $# -ne 1 ]
then
    echo "Usage: ${0} url_to_dlmat_installation" 1>&2
    exit 1
fi
dlmat_URL=$1
...
then
    ...
    echo 'Please install Python for the dlmat server to be able to run.' 2>&1
    exit 1
fi
...
${SCRIPT_DIR}/troubleshooting.py ${dlmat_URL}
