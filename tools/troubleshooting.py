...
"""Attempt to diagnose common problems with the dlmat server by using HTTP.
...
def main(args):
    ...
    if len(args) != 2:
        print('Usage: %s url_to_dlmat_installation' % (args[0], ), file=stderr)
        return -1
    dlmat_url = args[1]
    url_soup = urlparse(dlmat_url)
    ...
    if res.status != OK:
        print(('Unable to load "%s", please check the url.'
                          ) % (dlmat_url, ), file=stderr)
        print(('Does the url you provdide point to your dlmat '
    ...
    if res.status == FORBIDDEN:
        ...
        print(('Unable to load "%s", please check your url. Does '
                          'it point to your dlmat installation?') % (ajax_cgi_url, ), file=stderr)
        return -1
    ...
    print('Congratulations! Your dlmat server appears to be ready to run.')
