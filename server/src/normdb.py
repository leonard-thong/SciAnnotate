...
try:
    ...
    # for CLI use; assume we're in dlmat server/src/ and config is in root
...
def __db_path(db):
    ...
    contain the DB."""
    # Assume we have a path relative to the dlmat root if the value
    # contains a separator, name only otherwise.
    # TODO: better treatment of name / path ambiguity, this doesn't
    # allow e.g. DBs to be located in dlmat root
    if path_sep in db:
        base = BASE_DIR
    else:
        base = WORK_DIR
