...
"""Run dlmat using the built-in Python CGI server for testing purposes.
...
class dlmatCGIHTTPRequestHandler(CGIHTTPRequestHandler):
    def is_cgi(self):
...
def main(args):
    ...
    print(('    You can also use it for trying out dlmat before '
                      'deploying on a "real" web server such as Apache.'), file=stderr)
    print(('    Using this web server to run dlmat on an open '
    ...
    print(file=stderr)
    simple_http_server_test(dlmatCGIHTTPRequestHandler, HTTPServer)
