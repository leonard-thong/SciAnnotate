...
var AnnotatorUI = (function ($, window, undefined) {
    var AnnotatorUI = function (dispatcher, svg) {
        ...
        var makeSelRect = function (rx, ry, rw, rh, col) {
            var selRect = document.createElementNS(
            ...
            selRect.setAttributeNS(
                null,
                "fill",
                col == undefined ? "lightblue" : col
            );
            return selRect;
        };
        ...
        var onKeyDown = function (evt) {
            ...
            if (
                Configuration.rapidModeOn &&
                rapidAnnotationDialogVisible &&
                "0".charCodeAt() <= code &&
                code <= "9".charCodeAt()
            ) {
            ...
            if (
                nodeName == "input" &&
                (nodeType == "text" || nodeType == "password")
            )
                return;
            ...
            if (prefix + key == "C-H") {
                if (args.matchfocus) {
                    ...
                    if (valency == 2) {
                        // text offsets
                        if (spanOptions) {
                            // previous span edit exists
                            if (
                                spanOptions.action == "createSpan" &&
                                !spanOptions.id
                            ) {
                                // new
                                spanOptions.offsets = args.matchfocus;
                                spanFormSubmit();
                                dispatcher.post("logAction", [
                                    "spanSearchCreated",
                                ]);
                            } else {
                                // edit
                                var spanId = Object.keys(data.spans).find(
                                    function (spanId) {
                                        return Util.isEqual(
                                            data.spans[spanId].offsets,
                                            args.matchfocus
                                        );
                                    }
                                );
                            ...
                            // no previous span edit, so bring up the dialog
                            var spanText = data.text.substring(
                                args.matchfocus[0][0],
                                args.matchfocus[0][1]
                            );
                            spanOptions = {
                        ...
                        // relationship; XXX TODO not sure what to do with this
                    }
                }
                evt.preventDefault();
                return false;
        ...
        var onDblClick = function (evt) {
            ...
            if ((id = target.attr("data-arc-role"))) {
                ...
                if (eventDescId) {
                    ...
                    arcOptions.comment =
                        eventDesc.comment && eventDesc.comment.text;
                    if (eventDesc.equiv) {
                    ...
                    arcOptions.id =
                        originSpanId + "~" + type + "~" + targetSpanId;
                }
                $("#arc_origin").text(
                    Util.spanDisplayForm(spanTypes, originSpan.type) +
                        ' ("' +
                        originSpan.text +
                        '")'
                );
                $("#arc_target").text(
                    Util.spanDisplayForm(spanTypes, targetSpan.type) +
                        ' ("' +
                        targetSpan.text +
                        '")'
                );
                var arcId = eventDescId || [originSpanId, type, targetSpanId];
                fillArcTypesAndDisplayForm(
                    evt,
                    originSpan.type,
                    targetSpan.type,
                    type,
        ...
        var onMouseMove = function (evt) {
            if (arcDragOrigin) {
                if (arcDragJustStarted) {
                    ...
                    var noNumArcType = stripNumericSuffix(
                        arcOptions && arcOptions.type
                    );
                    var targetTypes = [];
                    $.each(
                        spanDesc.arcs || [],
                        function (possibleArcNo, possibleArc) {
                            if (
                                (arcOptions &&
                                    possibleArc.type == noNumArcType) ||
                                !(arcOptions && arcOptions.old_target)
                            ) {
                                $.each(
                                    possibleArc.targets || [],
                                    function (
                ...
                var chunkIndexFrom =
                    sel.anchorNode &&
                    $(sel.anchorNode.parentNode).attr("data-chunk-id");
                var chunkIndexTo =
                    sel.focusNode &&
                    $(sel.focusNode.parentNode).attr("data-chunk-id");
                ...
                if (
                    ...
                    while (tries < 2) {
                        ...
                        sp.x =
                            (flip ? evt.pageX : dragStartedAt.pageX) -
                            svgOffset.left;
                        sp.y =
                            (flip ? evt.pageY : dragStartedAt.pageY) -
                            (svgOffset.top + 8);
                        var startsAt = range.startContainer;
                        anchorOffset = startsAt.getCharNumAtPosition(sp);
                        chunkIndexFrom =
                            startsAt && $(startsAt).attr("data-chunk-id");
                        if (anchorOffset != -1) {
                            break;
                        }
                        flip = true;
                    ...
                    sp.x =
                        (flip ? dragStartedAt.pageX : evt.pageX) -
                        svgOffset.left;
                    sp.y =
                        (flip ? dragStartedAt.pageY : evt.pageY) -
                        (svgOffset.top + 8);
                    ...
                    if (
                        lastStartRec != null &&
                        lastStartRec.x == startRec.x &&
                        lastStartRec.y == startRec.y &&
                        lastEndRec != null &&
                        lastEndRec.x == endRec.x &&
                        lastEndRec.y == endRec.y
                    ) {
                        return;
                    }
                    ...
                    if (selRect == null) {
                        ...
                        if (
                            startRec.x != lastStartRec.x &&
                            endRec.x != lastEndRec.x &&
                            (startRec.y != lastStartRec.y ||
                                endRec.y != lastEndRec.y)
                        ) {
                            if (startRec.y < lastStartRec.y) {
                                selRect[0].setAttributeNS(
                                    null,
                                    "width",
                                    lastStartRec.width
                                );
                                ...
                                selRect[selRect.length - 1].setAttributeNS(
                                    null,
                                    "x",
                                    parseFloat(
                                        selRect[
                        ...
                        if (
                            ...
                            for (; ss != selRect.length; ss++) {
                                if (
                                    startRec.y <=
                                    parseFloat(
                                        selRect[ss].getAttributeNS(null, "y")
                                    )
                                ) {
                                    break;
                                }
                            ...
                            for (; es != -1; es--) {
                                if (
                                    endRec.y >=
                                    parseFloat(
                                        selRect[es].getAttributeNS(null, "y")
                                    )
                                ) {
                                    break;
                                }
                            ...
                            if (ss < selRect.length) {
                                for (var s2 = 0; s2 != ss; s2++) {
                                    selRect[s2].parentNode.removeChild(
                                        selRect[s2]
                                    );
                                    es--;
                                    trunc = true;
                            ...
                            if (trunc) {
                                var activeSelRect = flip
                                    ? selRect[0]
                                    : selRect[selRect.length - 1];
                                if (flip) {
                                    var rw = 0;
                                ...
                                if (flip) {
                                    lastSel.setAttributeNS(
                                        ...
                                        "width",
                                        parseFloat(
                                            lastSel.getAttributeNS(null, "x")
                                        ) +
                                            parseFloat(
                                                lastSel.getAttributeNS(
                                                    null,
                                                    "width"
                                                )
                                            ) -
                            ...
                            if (flip) {
                                var rw =
                                    parseFloat(
                                        activeSelRect.getAttributeNS(null, "x")
                                    ) +
                                    parseFloat(
                                        activeSelRect.getAttributeNS(
                                            null,
        ...
        var fillSpanTypesAndDisplayForm = function (evt, spanText, span) {
            ...
            $.each(searchConfig, function (searchNo, search) {
                $("#span_" + search[0]).attr(
                    "href",
                    search[1].replace("%s", encodedText)
                );
            });
            ...
            if (
                $("#entity_types .item").length >
                Configuration.typeCollapseLimit
            ) {
                $("#entity_types .open").removeClass("open");
            }
            if (
                $("#event_types .item").length > Configuration.typeCollapseLimit
            ) {
            ...
            if (span) {
                var linkHash = new URLHash(coll, doc, {
                    focus: [[span.id]],
                }).getHash();
                ...
                if (el.length) {
                    ...
                    $("#span_form input:radio:checked").each(function (
                        radioNo,
                        radio
                    ) {
                        radio.checked = false;
                ...
                $.each(span.outgoing, function (arcNo, arc) {
                    ...
                    if (
                        (arcTypeCount[noNumArcType] =
                            (arcTypeCount[noNumArcType] || 0) + 1) == 2
                    ) {
                ...
                var linkHash = new URLHash(coll, doc, {
                    focus: [[offsets[0], offsets[1]]],
                }).getHash();
                var firstRadio = $(
                    "#span_form input:radio:not([unused]):first"
                )[0];
                if (firstRadio) {
                    ...
                    dispatcher.post("messages", [
                        [["No valid span types defined", "error"]],
                    ]);
                    return;
                }
                $("#span_form_split").hide();
            ...
            if (span && !reselectedSpan) {
                $(
                    "#span_form_reselect, #span_form_delete, #span_form_add_fragment"
                ).show();
            ...
            if (span && !reselectedSpan && span.offsets.length > 1) {
                $(
                    "#span_form_reselect_fragment, #span_form_delete_fragment"
                ).show();
                keymap["S-" + $.ui.keyCode.DELETE] =
                    "span_form_delete_fragment";
                keymap["S-" + $.ui.keyCode.INSERT] =
                    "span_form_reselect_fragment";
            } else {
                $(
                    "#span_form_reselect_fragment, #span_form_delete_fragment"
                ).hide();
            ...
            if (!span) {
                ...
                var attrCategoryAndTypes = [
                    ["entity", entityAttributeTypes],
                ...
                if (span.generalType == "entity") {
                    ...
                    console.error(
                        "Unrecognized generalType:",
                        span.generalType
                    );
                }
                $.each(attributeTypes, function (attrNo, attr) {
                    $input = $(
                        "#" + category + "_attr_" + Util.escapeQuotes(attr.type)
                    );
            ...
            var showAttributesFor = function (attrTypes, category, type) {
                ...
                $.each(attrTypes, function (attrNo, attr) {
                    ...
                    var showAttr =
                        showAllAttributes ||
                        $.inArray(attr.type, validAttrs) != -1;
                    if (showAttr) {
            ...
            showValidAttributes = function () {
                ...
                var entityAttrCount = showAttributesFor(
                    entityAttributeTypes,
                    "entity",
                    type
                );
                var eventAttrCount = showAttributesFor(
                    eventAttributeTypes,
                    "event",
                    type
                );
                ...
                .append(
                    $.map(normCurrent, function (normalization, index) {
                        var allowed =
                            $.inArray(
                                normalization[0],
                                normAllowedNormalizations
                            ) != -1;
                        var allowedHTML = allowed ? "" : "&#x2757;";
                        var base = normDbUrlBaseByDbName[normalization[0]];
                        var $linkTd = $("<td/>");
                        if (base) {
                            var link = base.replace(
                                "%s",
                                encodeURIComponent(normalization[1])
                            );
                            $("<img/>")
                                .attr({
                                    src:
                                        "static/img/Fugue-shadowless-magnifier.png",
        ...
        var rapidFillSpanTypesAndDisplayForm = function (
            start,
            end,
            text,
            types
            ...
            $.each(types, function (typeNo, typeAndProb) {
                ...
                var $input = $('<input type="radio" name="rapid_span_type"/>')
                    ...
                    .attr("value", type);
                var spanBgColor =
                    (spanTypes[type] && spanTypes[type].bgColor) || "#ffffff";
                spanBgColor = Util.adjustColorLightness(
                    spanBgColor,
                    spanBoxTextBgColorLighten
                );
                ...
                if (typeHotkey) {
                    ...
                    var replace = true;
                    name = name.replace(
                        new RegExp("(&[^;]*?)?(" + typeHotkey + ")", "gi"),
                        function (all, entity, letter) {
                            if (replace && !entity) {
                                replace = false;
                                var hotkey =
                                    typeHotkey.toLowerCase() == letter
                                        ? typeHotkey.toLowerCase()
                                        : typeHotkey.toUpperCase();
                                return (
                                    '<span class="accesskey">' +
                                    Util.escapeHTML(hotkey) +
                                    "</span>"
                                );
                            }
                            return all;
            ...
            var $input = $(
                '<input type="radio" name="rapid_span_type" id="rapid_span_0" value=""/>'
            );
            var $label = $(
                '<label class="span_type_label" for="rapid_span_0" style="background-color:lightgray">Other...</label>'
            );
            var $content = $('<div class="item_content"/>')
                .append($numlabel)
                .append($input)
            ...
            rapidSpanForm
                .find("#rapid_span_types input:radio")
                .click(rapidSpanFormSubmitRadio);
        ...
        normListDialog.on("click", "tr", function (evt) {
            ...
            var allowed =
                $.inArray(normalization[0], normAllowedNormalizations) != -1;
            $("#norm_delete").show();
            $("#norm_edit").toggle(allowed);
        });
        normListDialog.submit(normListDialogSubmit);
        dispatcher.post("initForm", [
            normListDialog,
            {
                width: 600,
                width: 400,
                ...
                open: function (evt) {
                    ...
                    $("#norm_add").toggle(
                        normAllowedNormalizations &&
                            normAllowedNormalizations.length > 0
                    );
                    normEditedIndex = undefined;
                    $("#norm_list_dialog-ok").focus();
                },
        ...
        var checkAllowedNormalizations = function () {
            var spanType = $(
                "#entity_and_event_wrapper .item input:checked"
            ).val();
            normAllowedNormalizations = normDbsByType[spanType];
            $("#norm_qadd_button").toggle(
                normAllowedNormalizations &&
                    normAllowedNormalizations.length > 0
            );
        };
        ...
        $("#norm_search_query")
            .autocomplete({
                source: function (request, callback) {
                    ...
                    callback(
                        $.grep(lastNormSearches, function (search) {
                            return (
                                pattern.test(search.value) ||
                                pattern.test(search.id)
                            );
                        })
                    );
                },
        ...
        var setSpanNormSearchResults = function (response) {
            ...
            $.each(response.items, function (itemNo, item) {
                ...
                html.push(
                    "<tr" +
                        ' data-id="' +
                        Util.escapeHTMLandQuotes(item[0]) +
                        '"' +
                        ' data-txt="' +
                        Util.escapeHTMLandQuotes(item[1]) +
                        '"' +
                        ">"
                );
                for (var i = 0; i < len; i++) {
            ...
            $("#norm_search_result_select tbody")
                .find("tr")
                .click(chooseNormId)
        ...
        $("#norm_db_link").hide();
        $("#norm_db_choice")
            .addClass("ui-widget ui-state-default ui-button-text")
            .change(updateNormDbLink);
        ...
        var fillArcTypesAndDisplayForm = function (
            evt,
            originType,
            targetType,
            arcType,
            ...
            if (spanTypes[originType]) {
                ...
                $.each(arcTypes || [], function (arcTypeNo, arcDesc) {
                    if (
                        arcDesc.targets &&
                        arcDesc.targets.indexOf(targetType) != -1
                    ) {
                        var arcTypeName = arcDesc.type;
                        var relType =
                            relationTypesHash && relationTypesHash[arcTypeName];
                        ...
                        var displayName =
                            (arcDesc.labels && arcDesc.labels[0]) ||
                            arcTypeName;
                        var $input = $(
                            '<input id="arc_' +
                                arcTypeName +
                                '" type="radio" name="arc_type" value="' +
                                arcTypeName +
                                '"/>'
                        );
                        var $label = $(
                            '<label class="arc_type_label" for="arc_' +
                                arcTypeName +
                                '"/>'
                        ).text(displayName);
                        ...
                        if (
                            relType &&
                            relType.children &&
                            relType.children.length
                        ) {
                            var $collapser = $('<div class="collapser"/>').data(
                                "rel-name",
                                arcTypeName
                            );
                ...
                $.each(mapOf$items, function (name, $div) {
                    ...
                    $div.appendTo(
                        parent ? mapOf$containers[parent] : $scroller
                    );
                });
            }
            ...
            if (noArcs) {
                if (arcId) {
                    ...
                    var $checkbox = $(
                        '<input id="arc_' +
                            arcType +
                            '" type="hidden" name="arc_type" value="' +
                            noNumArcType +
                            '"/>'
                    );
            ...
            if (arcId) {
                ...
                if (backTargetType) {
                    $.each(
                        backTargetType.arcs || [],
                        function (backArcTypeNo, backArcDesc) {
                            if (
                                $.inArray(
                                    originType,
                                    backArcDesc.targets || []
                                ...
                                var relType =
                                    relationTypesHash[backArcDesc.type];
                                reversalPossible =
                                    relType &&
                                    relType.properties &&
                                    !relType.properties.symmetric;
                ...
                $(
                    "#arc_form_reselect, #arc_form_delete, #arc_form_reverse"
                ).hide();
            ...
            if (!Configuration.confirmModeOn) {
                arcForm
                    .find("#arc_roles input:radio")
                    .click(arcFormSubmitRadio);
            }
        ...
        $("#arc_form_reselect").attr(
            "title",
            "Re-select the annotation this connects into."
        );
        ...
        var tryToAnnotate = function (evt) {
            ...
            if (
                ...
                sp.x =
                    (flip ? dragStartedAt.pageX : evt.pageX) - svgOffset.left;
                sp.y =
                    (flip ? dragStartedAt.pageY : evt.pageY) -
                    (svgOffset.top + 8);
                var endsAt = range.endContainer;
            ...
            if (evt.type == "keydown") {
                ...
                var endpos = theFocusNode.parentNode.getEndPositionOfChar(
                    offset
                );
            ...
            if (chunkIndexFrom !== undefined && chunkIndexTo !== undefined) {
                ...
                if (reselectedSpan) {
                    ...
                    spanOptions.old_offsets = JSON.stringify(
                        reselectedSpan.offsets
                    );
...
*/
        ...
        var onMouseUp = function (evt) {
            ...
            if (
                !(
                    targetSpanId !== undefined ||
                    targetChunkId !== undefined ||
                    targetArcRole !== undefined
                )
            ) {
                // misclick
                clearSelection();
            ...
            if (
                arcDragJustStarted &&
                (Util.isMac ? evt.metaKey : evt.ctrlKey)
            ) {
        ...
        var receivedSuggestedSpanTypes = function (sugg) {
            ...
            if (sugg.collection != coll || sugg.document != doc) {
                dispatcher.post("messages", [
                    [
                        [
                            "Error: collection/document mismatch for span suggestions",
                            "error",
                        ],
                    ],
                ]);
            ...
            rapidFillSpanTypesAndDisplayForm(
                sugg.start,
                sugg.end,
                sugg.text,
                sugg.types
            ...
            $.each(types, function (typeNo, type) {
                if (type === null) {
                    ...
                    if (type.hotkey) {
                        ...
                        var replace = true;
                        name = name.replace(
                            new RegExp("(&[^;]*?)?(" + type.hotkey + ")", "gi"),
                            function (all, entity, letter) {
                                if (replace && !entity) {
                                    replace = false;
                                    var hotkey =
                                        type.hotkey.toLowerCase() == letter
        ...
        var addAttributeTypesToDiv = function ($top, types, category) {
            $.each(types, function (attrNo, attr) {
                ...
                var $span = $('<span class="attribute_type_label"/>').appendTo(
                    $top
                );
                if (attr.unused) {
                    $(
                    ...
                    var $select = $(
                        '<select id="' +
                            attrId +
                            '" class="ui-widget ui-state-default ui-button-text" category="' +
                            category +
                            '"/>'
                    );
                    var $option = $(
                        '<option class="ui-state-default" value=""/>'
                    ).text("?");
                    $select.append($option);
                    $.each(attr.values, function (valueNo, value) {
                        $option = $(
                            '<option class="ui-state-active" value="' +
                                Util.escapeQuotes(value.name) +
                                '"/>'
                        ).text(value.name);
                        $select.append($option);
                    });
        ...
        var onBooleanAttrChange = function (evt) {
            if (evt.type == "change") {
                // ignore the click event on the UI element
                var attrCategory = evt.target.getAttribute("category");
                setSpanTypeSelectability(attrCategory);
        ...
        var rememberSpanSettings = function (response) {
            ...
            spanForm
                .find("#entity_types input:radio")
                .click(spanFormSubmitRadio);
            spanForm
                .find("#event_types input:radio")
                .click(spanFormSubmitRadio);
        };
            ...
            $.each(taggers, function (taggerNo, tagger) {
                ...
                if (!taggerId || !taggerName || !taggerModel) {
                    dispatcher.post("messages", [
                        [
                            [
                                "Invalid tagger specification received from server",
                                "error",
                            ],
                        ],
                    ]);
                ...
                var $label = $(
                    '<span class="optionLabel">' +
                        Util.escapeHTML(taggerName) +
                        "</span>"
                );
                var $button = $(
                    '<input id="tag_' +
                        Util.escapeHTML(taggerId) +
                        '_button" type="button" value="' +
                        Util.escapeHTML(taggerModel) +
                        '" tabindex="-1" title="Automatically tag the current document."/>'
                );
        ...
        var clearNormalizationUI = function () {
            ...
            $("#norm_search_query").val("");
            $(
                "#norm_search_result_select thead, #norm_search_result_select tbody"
            ).empty();
        };
        ...
        var spanAndAttributeTypesLoaded = function (
            _spanTypes,
        ...
        var edited = function (response) {
            ...
            if (x) {
                ...
                if (response.edited == undefined) {
                    console.warn(
                        "Warning: server response to edit has",
                        response.edited,
                        'value for "edited"'
                    );
                } else {
        ...
        var deleteFragment = function () {
            if (
                Configuration.confirmModeOn &&
                !confirm("Are you sure you want to delete this fragment?")
            ) {
                return;
            }
            ...
            spanForm,
            {
                ...
                create: function (evt) {
                    var $ok = $("#span_form-ok").wrap(
                        '<span id="span_form_lock_bset"/>'
                    );
                    var $span = $ok.parent();
                    var $lock = $(
                        '<input id="span_form_lock" type="checkbox"/>'
                    ).insertBefore($ok);
                    $('<label for="span_form_lock"/>')
                        .text("Lock type")
                        .insertBefore($ok);
                    $lock.button({
                        id: "span_form_lock",
        ...
        // set button tooltips (@amadanmath: can this be done in init?)
        $("#span_form_reselect").attr(
            "title",
            "Re-select the text span that this annotation marks."
        );
        $("#span_form_delete").attr("title", "Delete this annotation.");
        $("#span_form_split").attr(
            "title",
            "Split this annotation into multiple similar annotations, distributing its arguments."
        );
        ...
        var rapidSpanFormSubmit = function (evt, typeRadio) {
            ...
            if (type == "") {
                ...
                $("#waiter").dialog("open");
                rapidSpanOptions.offsets = JSON.stringify(
                    rapidSpanOptions.offsets
                );
                    ...
                    if (x) {
                        if (x == "fileExistsError") {
                            dispatcher.post("messages", [
                                [
                                    [
                                        "A file with the given name exists. Please give a different name to the file to import.",
                                        "error",
                                    ],
                                ],
                            ]);
                            ...
                            dispatcher.post("messages", [
                                [
                                    [
                                        "Unknown error: " + response.exception,
                                        "error",
                                    ],
                                ],
                            ]);
        ...
        $("#delete_document_button").click(function () {
            ...
            if (
                !confirm(
                    "Are you sure you want to permanently remove this document and its annotations from the collection? This action cannot be undone."
                )
            ) {
                return;
            }
        ...
        $("#delete_collection_button").click(function () {
            ...
            if (
                !confirm(
                    "Are you sure you want to permanently REMOVE the ENTIRE COLLECTION " +
                        coll +
                        ", including all its documents and their annotations?  This action CANNOT BE UNDONE."
                )
            ) {
                return;
            }
        ...
        $("#undo_button").click(function () {
            if (coll && doc) {
                if (undoStack.length > 0) {
                    ...
                    dispatcher.post("messages", [
                        [["No action to be undone", "error"]],
                    ]);
                }
            } else {
                dispatcher.post("messages", [
                    [["No document loaded, can not undo changes", "error"]],
                ]);
            }
        });
