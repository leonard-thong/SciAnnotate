...
var Util = (function (window, undefined) {
    ...
    var arcDisplayForm = function (
        spanTypes,
        spanType,
        arcType,
        relationTypesHash
    ) {
        var labels = getArcLabels(
            spanTypes,
            spanType,
            arcType,
            relationTypesHash
    ...
    var paramArray = function (val) {
        ...
        for (var i = 0; i < len; i++) {
            if ($.isArray(val[i])) {
                ...
                console.error(
                    "param: Error: received non-array-in-array argument [",
                    i,
                    "]",
                    ":",
                    val[i],
        ...
        for (var key in args) {
            if (args.hasOwnProperty(key)) {
                ...
                if ($.isArray(val)) {
                    ...
                    // non-array argument; this is an error from the caller
                    console.error(
                        "param: Error: received non-array argument",
                        key,
                        ":",
                        val,
                        "(fix caller)"
    ...
    //   http://.../dlmat/ajax.cgi?action=getCollectionInformation&collection=...
    // docData: the document data (in the format of the result of
    //   http://.../dlmat/ajax.cgi?action=getDocument&collection=...&document=...
    ...
    var loadFonts = function (webFontURLs, dispatcher) {
        ...
        $.each(webFontURLs, function (urlNo, url) {
            ...
            else if (/PT.*Sans.*Caption/i.test(url))
                families.push("PT Sans Caption");
            else if (/Liberation.*Sans/i.test(url))
                families.push("Liberation Sans");
        });
    ...
    UnicodeString.prototype.toString = UnicodeString.prototype.valueOf = function () {
        return this.value;
    };
