...
var VisualizerUI = (function ($, window, undefined) {
    var VisualizerUI = function (dispatcher, svg) {
                ...
                $.each(msgs, function (msgNo, msg) {
                    ...
                    try {
                        ...
                        escaped = msg[0]
                            ...
                            .replace(/>/g, "&gt;");
                        element = $(
                            '<div class="error"><b>[ERROR: could not display the following message normally due to malformed XML:]</b><br/>' +
                                escaped +
                                "</div>"
                        );
                    }
        ...
        var displayComment = function (
            ...
            if (commentType) {
                ...
                if (commentType == "AnnotatorNotes") {
                    ...
                    commentLabel =
                        "<b>" + Util.escapeHTML(commentType) + ":</b> ";
                }
                comment +=
                    commentLabel + Util.escapeHTMLwithNewlines(commentText);
           ...
           before the popup obstructs them. */
            displayCommentTimer = setTimeout(
                function () {
        ...
        var displaySpanComment = function (
            ...
            var comment =
                '<div><span class="comment_type_id_wrapper">' +
                '<span class="comment_type">' +
                Util.escapeHTML(Util.spanDisplayForm(spanTypes, spanType)) +
                "</span>" +
                " " +
                '<span class="comment_id">' +
                "ID:" +
                Util.escapeHTML(spanId) +
                "</span></span>";
            if (mods.length) {
                comment +=
                    "<div>" + Util.escapeHTML(mods.join(", ")) + "</div>";
            }
            ...
            comment +=
                ...
                '"</div>';
            var validArcTypesForDrag = dispatcher.post(
                "getValidArcTypesForDrag",
                [spanId, spanType]
            );
            if (validArcTypesForDrag && validArcTypesForDrag[0]) {
                if (validArcTypesForDrag[0].length) {
                    comment +=
                        "<div>" + validArcTypesForDrag[0].join(", ") + "</div>";
                } else {
                    $('rect[data-span-id="' + spanId + '"]').addClass(
                        "badTarget"
                    );
                }
                immediately = true;
            ...
            $.each(normalizations, function (normNo, norm) {
                ...
                comment +=
                    ...
                    ":" +
                    Util.escapeHTML(dbKey) +
                    "</span>";
                if (
                    dbName in normServerDbByNormDbName &&
                    ...
                    if (norm[2]) {
                        comment +=
                            '<br/><span class="norm_info_value">' +
                            Util.escapeHTML(norm[2]) +
                            "</span>";
                    }
                }
            });
        ...
        var displayArcComment = function (
            ...
            var arcDisplayForm = Util.arcDisplayForm(
                spanTypes,
            ...
            comment +=
                ...
                '<span class="comment_type">' +
                Util.escapeHTML(
                    Util.spanDisplayForm(spanTypes, originSpanType)
                ) +
                " " +
                arrowStr +
                " " +
                ...
                " " +
                Util.escapeHTML(
                    Util.spanDisplayForm(spanTypes, targetSpanType)
                ) +
                "</span>" +
                '<span class="comment_id">' +
                (arcId
            ...
            if (opts.label_keyword_option) {
                buttons.push({
                    ...
                    click: function () {
                        ...
                        let option = $("#label_scope input:radio:checked")
                            .map(function () {
                                return $(this).val();
                            })
                            .get();
            ...
            if (opts.label_select_option) {
                buttons.push({
                    ...
                    click: function () {
                        ...
                        let functions = $(
                            "#label_form_select input:checkbox:checked"
                        )
                            .map(function () {
                                return $(this).val();
                            })
                        ...
                        if (functions.length >= 1) {
                            ...
                            dispatcher.post("messages", [
                                [
                                    [
                                        "Select at least one labeling function",
                                        "warning",
                                    ],
                                ],
                            ]);
        ...
        var unsafeDialogOpen = function ($dialog) {
            ...
            // https://github.com/nlplab/dlmat/issues/934
      ...
      */
        ...
        var fileBrowserSubmit = function (evt) {
            ...
            if (input.substr(0, 2) === "..") {
                ...
                dispatcher.post("messages", [
                    [["Invalid document name format", "error", 2]],
                ]);
                $("#document_input").focus().select();
            }
        ...
        var showFileBrowser = function () {
            ...
            $.each(selectorData.items, function (docNo, doc) {
                ...
                var annp = doc[1]
                    ? "?" + Util.escapeHTML(Util.param(doc[1]))
                    : "";
                ...
                var collFileImg = isColl
                    ? "Fugue-shadowless-folder-horizontal-open.png"
                    : "Fugue-shadowless-document.png";
                var collSuffix = isColl ? "/" : "";
                if (doc[1]) {
                    var matchfocus = doc[1].matchfocus || [];
                    var mfstr =
                        ' data-mf="' + Util.paramArray(matchfocus) + '"';
                    var match = doc[1].match || [];
                    var matchstr =
                        ' data-match="' + Util.paramArray(match) + '"';
                } else {
                    var matchstr = "";
                    var mfstr = "";
                }
        ...
        var currentSelectorPosition = function () {
            ...
            $.each(selectorData.items, function (docNo, docRow) {
                if (docRow[2] == doc) {
                    ...
                    if (
                        Util.isEqual(collectionArgs.matchfocus, args.matchfocus)
                    ) {
                        pos = docNo;
                        return false;
                    }
            ...
            $.each(types, function (typeNo, type) {
                if (type !== null) {
                    if (!included[type.name]) {
                        ...
                        var $option = $(
                            '<option value="' +
                                Util.escapeQuotes(type.type) +
                                '"/>'
                        ).text(type.name);
                        $select.append($option);
                        if (type.children) {
                            addSpanTypesToSelect(
                                $select,
                                type.children,
                                included
                            );
        ...
        var setupSearchTypes = function (response) {
            addSpanTypesToSelect(
                $("#search_form_entity_type"),
                response.entity_types
            );
            addSpanTypesToSelect(
                $("#search_form_event_type"),
            ...
            $("#search_form select").addClass(
                "ui-widget ui-state-default ui-button-text"
            );
            $("#search_form_load_file").addClass(
                "ui-widget ui-state-default ui-button-text"
            );
        };
        ...
        var searchEventRoleChanged = function (evt) {
            ...
            $.each(targets, function (targetNo, target) {
                ...
                var option =
                    '<option value="' +
                    Util.escapeQuotes(target) +
                    '">' +
                    Util.escapeHTML(spanName) +
                    "</option>";
                $type.append(option);
            });
        ...
        $("#search_form_event_roles").on(
            "change",
            ".search_event_role select",
            searchEventRoleChanged
        );
        ...
        var addEmptySearchEventRole = function () {
            ...
            $.each(searchEventRoles, function (arcTypePairNo, arcTypePair) {
                var option =
                    '<option value="' +
                    Util.escapeQuotes(arcTypePair[0]) +
                    '">' +
                    Util.escapeHTML(arcTypePair[1]) +
                    "</option>";
                $role.append(option);
            });
            ...
            button.button();
            button
                .addClass("small-buttons ui-button-text")
        ...
        $("#search_form_event_roles").on(
            "click",
            ".search_event_role_add input",
            addEmptySearchEventRole
        );
        $("#search_form_event_roles").on(
            "click",
            ".search_event_role_del input",
            delSearchEventRole
        );
        ...
        $("#search_form_event_type").change(function (evt) {
            ...
            $.each(arcTypes, function (arcTypeNo, arcType) {
                var arcTypeName =
                    (arcType.labels && arcType.labels[0]) || arcType.type;
        ...
        $("#search_form_relation_type").change(function (evt) {
            ...
            var $arg1 = $("#search_form_relation_arg1_type").html(
                ...
                if (spanType.arcs) {
                    $.each(spanType.arcs, function (arcTypeNo, arcType) {
                        if (arcType.type === relTypeType) {
                            ...
                            var option =
                                '<option value="' +
                                Util.escapeQuotes(spanTypeType) +
                                '">' +
                                Util.escapeHTML(spanName) +
                                "</option>";
                            $arg1.append(option);
        ...
        $("#search_form_relation_arg1_type").change(function (evt) {
            var $arg2 = $("#search_form_relation_arg2_type").html(
                '<option value="">- Any -</option>'
            );
            ...
            if (arcType && arcType.targets) {
                $.each(arcType.targets, function (spanTypeNo, spanTypeType) {
                    var spanName = Util.spanDisplayForm(
                        spanTypes,
                        spanTypeType
                    );
                    var option =
        ...
        $("#search_form_note_category").change(function (evt) {
            ...
            if ($.inArray(category, ["entity", "event", "relation"]) != -1) {
                $type
                    .html($("#search_form_" + category + "_type").html())
                    .val("");
            ...
            switch (action) {
                case "searchText":
                    ...
                    if (!opts.text.length) {
                        dispatcher.post("messages", [
                            [
                                [
                                    "Please fill in the text to search for!",
                                    "comment",
                                ],
                            ],
                        ]);
                ...
                case "searchEvent":
                    ...
                    $("#search_form_event_roles tr").each(function () {
                        var earg = {};
                        earg.role =
                            $(this).find(".search_event_role select").val() ||
                            "";
                        earg.type =
                            $(this).find(".search_event_type select").val() ||
                            "";
                        earg.text = $(this)
                            .find(".search_event_text input")
                            .val();
                        eargs.push(earg);
                    });
                ...
                case "searchRelation":
                    ...
                    opts.arg1type =
                        $("#search_form_relation_arg1_type").val() || "";
                    opts.arg2 = $("#search_form_relation_arg2_text").val();
                    opts.arg2type =
                        $("#search_form_relation_arg2_type").val() || "";
                    opts.show_text = $(
                        "#search_form_relation_show_arg_text_on"
                    ).is(":checked");
                    opts.show_type = $(
                        "#search_form_relation_show_arg_type_on"
                    ).is(":checked");
                    break;
                case "searchNote":
        ...
        $("#search_form_load_file").change(function (evt) {
            ...
            reader.onloadend = function (evt) {
                try {
                    ...
                    dispatcher.post("messages", [
                        [["The file contains invalid data.", "error"]],
                    ]);
                    return;
                }
            };
        ...
        $("#search_form_clear").attr(
            "title",
            "Clear the search and resume normal collection browsing"
        );
        ...
        var fillDisambiguatorOptions = function (disambiguators) {
            $("#annotation_speed3").button(
                disambiguators.length ? "enable" : "disable"
            );
        ...
        var onKeyDown = function (evt) {
            ...
            if (currentForm) {
                if (code === $.ui.keyCode.ENTER) {
                    ...
                    var nodeType = evt.target.type
                        ? evt.target.type.toLowerCase()
                        : "";
                    if (
                        evt.target.nodeName &&
        ...
        var moveInFileBrowser = function (dir) {
            ...
            if (
                newPos >= 0 &&
                newPos < selectorData.items.length &&
                selectorData.items[newPos][0] != "c"
            ) {
                // not at the start, and the previous is not a collection (dir)
                dispatcher.post("allowReloadByURL");
                dispatcher.post("setDocument", [
                    selectorData.items[newPos][2],
        ...
        var collectionLoaded = function (response) {
            if (response.exception) {
                if (
                    response.exception == "annotationCollectionNotFound" ||
                    ...
                    dispatcher.post("messages", [
                        [["Unknown error: " + response.exception, "error"]],
                    ]);
                    dispatcher.post("setCollection", ["/"]);
                }
            } else {
        ...
        var onDoneRendering = function (coll, doc, args) {
            if (args && !args.edited) {
                ...
                var $inFocus = $(
                    '#svg animate[data-type="focus"]:first'
                ).parent();
                if ($inFocus.length) {
                    $("html,body").animate(
                        {
        ...
        var onNewSourceData = function (sourceData) {
            ...
            $.each(sourceData.source_files, function (extNo, ext) {
                var $link = $('<a target="dlmat_search"/>')
                    .text(ext)
                    .attr(
                        "href",
                        "ajax.cgi?action=downloadFile&collection=" +
            ...
            if (mtime) {
                ...
                $("#document_mtime")
                    .text("Last modified: " + Util.formatTimeAgo(1000 * mtime))
                    .show();
            } else {
        ...
        var gotCurrent = function (_coll, _doc, _args) {
            ...
            if (oldColl != coll) {
                ...
                var $collectionDownloadLink = $('<a target="dlmat_search"/>')
                    .text("Download tar.gz")
                    .attr(
                        "href",
                        "ajax.cgi?action=downloadCollection&collection=" +
                            encodeURIComponent(coll) +
                            "&include_conf=" +
                            ($("#source_collection_conf_on").is(":checked")
                                ? 1
                                : 0) +
                            // TODO: Extract the protocol version into somewhere global
                            "&protocol=" +
        ...
        $("#label_abbreviations input").click(function (evt) {
            ...
            if (val) {
                dispatcher.post("messages", [
                    [["Abbreviations are now on", "comment"]],
                ]);
            } else {
                dispatcher.post("messages", [
                    [["Abbreviations are now off", "comment"]],
                ]);
            }
                    ...
                    if (response.config != undefined) {
                        ...
                        try {
                            ...
                            dispatcher.post("messages", [
                                [
                                    [
                                        "Corrupted configuration; resetting.",
                                        "error",
                                    ],
                                ],
                            ]);
        ...
        var showUnableToReadTextFile = function () {
            dispatcher.post("messages", [
                [["Unable to read the text file.", "error"]],
            ]);
            dispatcher.post("clearSVG");
            showFileBrowser();
        };
        ...
        var checkForDocumentChanges = function () {
                    ...
                    function (response) {
                        if (data) {
                            if (mtime != response.mtime) {
                                ...
                                if (
                                    documentChangesTimeout >=
                                    maxDocumentChangesTimeout
                                )
            ...
            documentChangesTimer = setTimeout(
                checkForDocumentChanges,
                documentChangesTimeout
            );
        };
        ...
        $("#autorefresh_mode").click(function (evt) {
            ...
            if (val) {
                ...
                checkForDocumentChanges();
                dispatcher.post("messages", [
                    [["Autorefresh mode is now on", "comment"]],
                ]);
            } else {
                Configuration.autorefreshOn = false;
                clearTimeout(documentChangesTimer);
                dispatcher.post("messages", [
                    [["Autorefresh mode is now off", "comment"]],
                ]);
            }
        ...
        var updateConfigurationUI = function () {
            ...
            // Text backgrounds
            $(
                '#text_backgrounds input[value="' +
                    Configuration.textBackgrounds +
                    '"]'
            )[0].checked = true;
            ...
            if (!splitSvgWidth) {
                ...
                $(
                    '#svg_width_unit input[value="' + splitSvgWidth[2] + '"]'
                )[0].checked = true;
        ...
        $("#source_collection_conf_on, #source_collection_conf_off").change(
            function () {
                var conf = $("#source_collection_conf_on").is(":checked")
                    ? 1
                    : 0;
                var $source_collection_link = $("#source_collection a");
                var link = $source_collection_link
                    .attr("href")
                    .replace(/&include_conf=./, "&include_conf=" + conf);
        ...
        dispatcher
            ...
            .on(
                "renderError:annotationFileNotFound",
                showAnnotationFileNotFound
            )
