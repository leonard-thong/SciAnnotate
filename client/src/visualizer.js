...
var Visualizer = (function ($, window, undefined) {
    ...
    var Visualizer = function (dispatcher, svgId, webFontURLs) {
        ...
        var setMarked = function (markedType) {
            $.each(args[markedType] || [], function (markedNo, marked) {
                if (marked[0] == "sent") {
                    ...
                    markedText.push([
                        parseInt(marked[0], 10),
                        parseInt(marked[1], 10),
                        markedType,
                    ]);
                } else {
                    ...
                    if (span) {
                        ...
                        if (eventDesc) {
                            ...
                            $.each(
                                data.spans[eventDesc.triggerId].outgoing,
                                function (arcNo, arc) {
                                    if (
                                        arc.target == relArc.targetId &&
                                        arc.type == relArc.type
                                    ) {
                                        arc.marked = markedType;
                                    }
                                }
                            ...
                            $.each(
                                data.eventDescs,
                                function (eventDescNo, eventDesc) {
        ...
        var adjustFragmentHeights = function (
            fromIndex,
            toIndex,
            fragmentHeights,
            height
        ...
        var setData = function (_sourceData) {
            ...
            $.each(sourceData.modifications, function (modNo, mod) {
                ...
                if (!data.spans[mod[2]]) {
                    dispatcher.post("messages", [
                        [
                            [
                                "<strong>ERROR</strong><br/>Event " +
                                    mod[2] +
                                    " (referenced from modification " +
                                    mod[0] +
                                    ") does not occur in document " +
                                    data.document +
                                    "<br/>(please correct the source data)",
                                "error",
                                5,
                            ],
                        ],
            ...
            $.each(sourceData.token_offsets, function () {
                ...
                if (
                    startFragmentId &&
                    to > sortedFragments[startFragmentId - 1].to
                ) {
                    while (
                ...
                if (
                    currentFragmentId < numFragments &&
                    to > sortedFragments[currentFragmentId].from
                ) {
                    return;
                }
            ...
            $.each(sourceData.sentence_offsets, function () {
                ...
                while (
                    chunkNo < numChunks &&
                    (chunk = data.chunks[chunkNo]).from < from
                ) {
                    chunk.outOfPage = pos;
                    chunkNo++;
                }
            ...
            while (chunkNo < numChunks) {
                data.chunks[chunkNo++].outOfPage = Configuration.pagingSize
                    ? pos
                    : 0;
            }
            ...
            $.each(sortedFragments, function (fragmentId, fragment) {
                ...
                if (pos == 0) {
                    if (!fragment.span.firstFragment)
                        fragment.span.firstFragment = fragment;
            ...
            $.each(sourceData.attributes, function (attrNo, attr) {
                ...
                var attrType =
                    eventAttributeTypes[attr[1]] ||
                    entityAttributeTypes[attr[1]];
                var attrValue =
                    attrType &&
                    attrType.values.byName[attrType.bool || attr[3]];
            ...
            $.each(sourceData.comments, function (commentNo, comment) {
                ...
                if (comment[0] instanceof Array && comment[0][0] == "sent") {
                    ...
                    $.each(commentEntities, function (entityId, entity) {
                        ...
                        if (
                            commentPriority(comment[1]) >
                            commentPriority(entity.shadowClass)
                        ) {
            ...
            $.each(data.eventDescs, function (eventNo, eventDesc) {
                ...
                $.each(eventDesc.roles, function (roleNo, role) {
                    ...
                    var there =
                        target.headFragment.from + target.headFragment.to;
            ...
            for (var i = 0; i < 2; i++) {
                ...
                $.each(data.arcs, function (arcNo, arc) {
                    data.spans[arc.origin].refedIndexSum +=
                        data.spans[arc.target].headFragment.indexNumber;
                });
            }
            ...
            data.spanDrawOrderPermutation.sort(function (a, b) {
                ...
                var tmp =
                    spanA.headFragment.drawOrder - spanB.headFragment.drawOrder;
            ...
            $.each(data.chunks, function (chunkNo, chunk) {
                ...
                $.each(chunk.fragments, function (fragmentNo, fragment) {
                    ...
                    var spanLabels = Util.getSpanLabels(
                        spanTypes,
                        fragment.span.type
                    );
                    fragment.labelText = Util.spanDisplayForm(
                        spanTypes,
                        fragment.span.type
                    );
                    ...
                    if (Configuration.abbrevsOn && spanLabels) {
                        ...
                        while (
                            fragment.labelText.length > maxLength &&
                            spanLabels[labelIdx]
                        ) {
                    ...
                    var warning = false;
                    $.each(
                        fragment.span.attributes,
                        function (attrType, valType) {
                            // TODO: might wish to check what's appropriate for the type
                            // instead of using the first attribute def found
                            var attr =
                                eventAttributeTypes[attrType] ||
        ...
        var showMtime = function () {
            if (data.mtime) {
                ...
                $("#document_mtime")
                    .text(
                        "Last modified: " +
                            Util.formatTimeAgo(1000 * data.mtime)
                    )
                    .css("display", "inline");
            } else {
        ...
        var addHeaderAndDefs = function () {
            ...
            var defs = svg.defs();
            var $blurFilter = $(
                '<filter id="Gaussian_Blur"><feGaussianBlur in="SourceGraphic" stdDeviation="2" /></filter>'
            );
        ...
        var getTextMeasurements = function (textsHash, options, callback) {
            ...
            $(textMeasureGroup)
                .find("text")
                .each(function (svgTextNo, svgText) {
            ...
            var textSizes = getTextMeasurements(
                ...
                function (fragment, text) {
                    if (fragment instanceof Fragment) {
                        ...
                        var textUpToFirstChar = fragment.chunk.text.substring(
                            0,
                            firstChar
                        );
                        var textUpToLastChar = fragment.chunk.text.substring(
                            0,
                            lastChar
                        );
                        var textUpToFirstCharUnspaced = textUpToFirstChar.replace(
                            /\s\s+/g,
                            " "
                        );
                        var textUpToLastCharUnspaced = textUpToLastChar.replace(
                            /\s\s+/g,
                            " "
                        );
                        firstChar -=
                            textUpToFirstChar.length -
                            textUpToFirstCharUnspaced.length;
                        lastChar -=
                            textUpToLastChar.length -
                            textUpToLastCharUnspaced.length;
                        ...
                        // it's markedText [id, start?, char#, offset, kind]
                        var textUpToChar = text.textContent.substring(
                            0,
                            fragment[2]
                        );
                        var textUpToCharUnspaced = textUpToChar.replace(
                            /\s\s+/g,
                            " "
                        );
                        var pos =
                            fragment[2] -
                            (textUpToChar.length - textUpToCharUnspaced.length);
                        if (pos < 0) pos = 0;
                        if (!pos) {
                            // start
                            ...
                            fragment[3] =
                                text.getEndPositionOfChar(pos - 1).x + 1;
                        }
                    }
                }
            );
        ...
        var addArcTextMeasurements = function (sizes) {
            ...
            var arcTexts = {};
            $.each(data.arcs, function (arcNo, arc) {
                var labels = Util.getArcLabels(
                    spanTypes,
                    data.spans[arc.origin].type,
                    arc.type,
                    relationTypesHash
                );
        ...
        var adjustTowerAnnotationSizes = function () {
            ...
            $.each(data.towers, function (towerNo, tower) {
                ...
                $.each(tower, function (fragmentNo, fragment) {
                    var width =
                        data.sizes.fragments.widths[fragment.glyphedLabelText];
          ...
          */
            ...
            for (var i = 0; i < styles.length; i++) {
                ...
                if (styleMap[targetRef][attribute] !== undefined) {
                    console.log(
                        "warning: multiple style definitions for",
                        targetRef,
                        attribute
                    );
                }
        ...
        var renderDataReal = function (sourceData) {
            ...
            if (rtlmode) {
                ...
                currentX =
                    Configuration.visual.margin.x + sentNumMargin + rowPadding;
            }
            ...
            $.each(data.spanDrawOrderPermutation, function (spanIdNo, spanId) {
                ...
                var bgColor =
                    ...
                    (spanTypes.SPAN_DEFAULT &&
                        spanTypes.SPAN_DEFAULT.bgColor) ||
                    "#ffffff";
                if (bgColor == "hidden") {
                    span.hidden = true;
                    return;
                ...
                let entity_name =
                    span.type.indexOf("_") === -1
                        ? span.type
                        : span.type[span.type.indexOf("_") + 1];
                ...
                var f1 = span.firstFragment,
                    f2 = span.lastFragment;
                ...
                var carpet = 0;
                var outside = true;
                var thisCurlyHeight = span.drawCurly
                    ? Configuration.visual.curlyHeight
                    : 0;
                var height =
                    sizes.fragments.height +
                    thisCurlyHeight +
            ...
            $.each(data.chunks, function (chunkNo, chunk) {
                ...
                if (chunk.lastSpace) {
                    ...
                    if (spacePos || !chunkNo || !chunk.sentence) {
                        ...
                        for (var i = spacePos; i < spaceLen; i++)
                            spaceWidth +=
                                spaceWidths[chunk.lastSpace.charAt(i)] || 0;
                ...
                $.each(chunk.fragments, function (fragmentNo, fragment) {
                    ...
                    var label_text =
                        data.spanAnnTexts[fragment.glyphedLabelText]
                            ._parts[0][1];
                    var function_name = null;
                    var entity_name = null;
                    if (label_text.indexOf("_") !== -1) {
                        function_name = label_text.slice(
                            0,
                            label_text.indexOf("_")
                        );
                        entity_name = label_text.slice(
                            label_text.indexOf("_") + 1
                        );
                    ...
                    var borderColor =
                        ...
                        (spanTypes.SPAN_DEFAULT &&
                            spanTypes.SPAN_DEFAULT.borderColor) ||
                        "#000000";
                    ...
                    var rectClass =
                        "span_" + (span.cue || span.type) + " span_default"; // TODO XXX first part unneeded I think; remove
                    ...
                    var bx =
                        xx - Configuration.visual.margin.x - boxTextMargin.x;
                    ...
                    fragment.rect = svg.rect(fragment.group, bx, by, bw, bh, {
                        ...
                        "data-fragment-id":
                            span.segmentedOffsetsMap[fragment.id],
                    ...
                    fragment.height =
                        ...
                        Configuration.visual.curlyHeight +
                        Configuration.visual.arcSpacing;
                    ...
                    $(fragment.rect).attr(
                        "y",
                        yy - Configuration.visual.margin.y - span.floor
                    );
                    if (shadowRect) {
                        $(shadowRect).attr(
                            "y",
                    ...
                    if (function_name) {
                        ...
                        svg.text(fragment.group, x, y - span.floor, tmpAnnObj, {
                            fill: fgColor,
                        });
                    } else svg.text(fragment.group, x, y - span.floor, data.spanAnnTexts[fragment.glyphedLabelText], { fill: fgColor });
                        ...
                        if (coloredCurlies) {
                            ...
                            var bgColor =
                                ...
                                (spanTypes.SPAN_DEFAULT &&
                                    spanTypes.SPAN_DEFAULT.fgColor) ||
                                "#000000";
                            curlyColor = Util.adjustColorLightness(
                                bgColor,
                                -0.6
                            );
                        }
                    ...
                    if (fragment == span.headFragment) {
                        ...
                        $.each(span.incoming, function (arcId, arc) {
                            ...
                            if (origin.row) {
                                ...
                                var labelNo = Configuration.abbrevsOn
                                    ? labels.length - 1
                                    : 0;
                                var smallestLabelWidth =
                                    sizes.arcs.widths[labels[labelNo]] +
                                    2 * minArcSlant;
                                var gap = Math.abs(
                                    currentX + (rtlmode ? -bx : bx) - border
                                );
                    ...
                    fragmentHeight +=
                        span.floor || Configuration.visual.curlyHeight;
                ...
                if (rtlmode) {
                    boxWidth =
                        Math.max(textWidth, -chunkFrom) - Math.min(0, -chunkTo);
                } else {
                    boxWidth =
                        Math.max(textWidth, chunkTo) - Math.min(0, chunkFrom);
                }
                // if (hasLeftArcs) {
                ...
                // }
                var rightBorderForArcs = hasRightArcs
                    ? arcHorizontalSpacing
                    : hasInternalArcs
                    ? arcSlant
                    : 0;
                var leftBorderForArcs = hasLeftArcs
                    ? arcHorizontalSpacing
                    : hasInternalArcs
                    ? arcSlant
                    : 0;
                ...
                if (rtlmode) {
                    chunkDoesNotFit =
                        currentX - boxWidth - leftBorderForArcs <=
                ...
                if (chunk.sentence || chunkDoesNotFit) {
                    ...
                    if (hasLeftArcs) {
                        var adjustedCurTextWidth =
                            sizes.texts.widths[chunk.text] +
                            arcHorizontalSpacing;
                ...
                if (spacing > 0) {
                    ...
                    if (firstChunkInRow) {
                        ...
                        for (
                            ...
                            translate(
                                movedChunk,
                                movedChunk.translation.x + spacing,
                                0
                            );
            ...
            data.arcs.sort(function (a, b) {
                ...
                tmp =
                    data.spans[a.origin].headFragment.height +
                    data.spans[a.target].headFragment.height -
                    data.spans[b.origin].headFragment.height -
                    data.spans[b.target].headFragment.height;
                if (tmp) return tmp < 0 ? -1 : 1;
            ...
            $.each(rows, function (rowId, row) {
                ...
                $.each(row.chunks, function (chunkId, chunk) {
                    if (chunk.lastFragmentIndex !== undefined) {
                        ...
                        seenFragment = true;
                        var heightsIndex =
                            chunk.lastFragmentIndex * 2 + heightsRowsAdded;
                        if (row.heightsEnd < heightsIndex) {
                            row.heightsEnd = heightsIndex;
                        }
                        var heightsIndex =
                            chunk.firstFragmentIndex * 2 + heightsRowsAdded;
                        if (row.heightsStart > heightsIndex) {
                            row.heightsStart = heightsIndex;
                        }
                    }
                });
                fragmentHeights.splice(
                    row.heightsStart,
                    0,
                    Configuration.visual.arcStartHeight
                );
                heightsRowsAdded++;
            ...
            $.each(data.arcs, function (arcNo, arc) {
                ...
                var leftToRight =
                    originSpan.headFragment.towerId <
                    targetSpan.headFragment.towerId;
                var left, right;
                if (leftToRight) {
                ...
                var symmetric =
                    arcDesc &&
                    arcDesc.properties &&
                    arcDesc.properties.symmetric;
                var dashArray = arcDesc && arcDesc.dashArray;
                var arrowHead =
                    (arcDesc && arcDesc.arrowHead) ||
                    (spanTypes.ARC_DEFAULT &&
                        spanTypes.ARC_DEFAULT.arrowHead) ||
                    "triangle,5";
                var labelArrowHead =
                    (arcDesc && arcDesc.labelArrow) ||
                    (spanTypes.ARC_DEFAULT &&
                        spanTypes.ARC_DEFAULT.labelArrow) ||
                    "triangle,5";
                ...
                if (!leftPos) var leftBox = rowBBox(left);
                if (!rightPos) var rightBox = rowBBox(right);
                var leftRow = left.chunk.row ? left.chunk.row.index : 0;
                var rightRow = right.chunk.row
                    ? right.chunk.row.index
                    : rows.length - 1;
                ...
                if (left.chunk.index == right.chunk.index) {
                    // always in page
                    fromIndex2 =
                        left.towerId * 2 + left.chunk.row.heightsAdjust;
                    toIndex2 =
                        right.towerId * 2 + right.chunk.row.heightsAdjust;
                } else {
                    // can be out of page
                    var leftChunk = left.chunk.outOfPage
                        ? firstChunk
                        : left.chunk;
                    var rightChunk = right.chunk.outOfPage
                        ? lastChunk
                        : right.chunk;
                    fromIndex2 =
                        left.towerId * 2 + 1 + leftChunk.row.heightsAdjust;
                    toIndex2 =
                        right.towerId * 2 - 1 + rightChunk.row.heightsAdjust;
                }
                if (!collapseArcSpace) {
                    height = findArcHeight(
                        fromIndex2,
                        toIndex2,
                        fragmentHeights
                    );
                ...
                var ufoCatcher =
                    originSpan.headFragment.chunk.index ==
                    targetSpan.headFragment.chunk.index;
                if (ufoCatcher) {
                    chunkReverse =
                        leftBox.x + leftBox.width / 2 <
                        rightBox.x + rightBox.width / 2;
                }
                var ufoCatcherMod = ufoCatcher
                    ? chunkReverse
                        ? -0.5
                        : 0.5
                    : 1;
                    ...
                    if (row.chunks.length) {
                        ...
                        if (leftBox && rowIndex == leftRow) {
                            if (rtlmode) {
                                from =
                                    leftBox.x +
                                    (chunkReverse ? leftBox.width : 0);
                            } else {
                                from =
                                    leftBox.x +
                                    (chunkReverse ? 0 : leftBox.width);
                            }
                        } else {
                            from = rtlmode
                                ? canvasWidth -
                        ...
                        if (rightBox && rowIndex == rightRow) {
                            if (rtlmode) {
                                to =
                                    rightBox.x +
                                    (chunkReverse ? 0 : rightBox.width);
                            } else {
                                to =
                                    rightBox.x +
                                    (chunkReverse ? rightBox.width : 0);
                            }
                        } else {
                            to = rtlmode
                                ? 0
                        ...
                        var arcLabels = Util.getArcLabels(
                            spanTypes,
                            originType,
                            arc.type,
                            relationTypesHash
                        );
                        var labelText = Util.arcDisplayForm(
                            spanTypes,
                            originType,
                            arc.type,
                            relationTypesHash
                        ...
                        if (Configuration.abbrevsOn && arcLabels) {
                            ...
                            while (
                                sizes.arcs.widths[labelText] > maxLength &&
                                arcLabels[labelIdx]
                            ) {
                        ...
                        if (!splitArcType[2]) {
                            ...
                            var splitLabelText = labelText.match(
                                /^(.*?)(\d*)$/
                            );
                        ...
                        if (arc.marked) {
                            var markedRect = svg.rect(
                                shadowGroup,
                                textBox.x - markedArcSize,
                                textBox.y - markedArcSize,
                                textBox.width + 2 * markedArcSize,
                                textBox.height + 2 * markedArcSize,
                                {
                                    // filter: 'url(#Gaussian_Blur)',
                                    class: "shadow_EditHighlight",
                                    rx: markedArcSize,
                        ...
                        var myLabelArrowHead =
                            (styleMap[arcRef] &&
                                styleMap[arcRef]["labelArrow"]) ||
                            (arcDesc && arcDesc.labelArrow) ||
                            (spanTypes.ARC_DEFAULT &&
                                spanTypes.ARC_DEFAULT.labelArrow);
                        if (myLabelArrowHead) {
                            ...
                            arrowAtLabelAdjust =
                                (labelArrowSplit[0] != "none" &&
                                    parseInt(labelArrowSplit[1], 10)) ||
                                0;
                        ...
                        if (leftBox && rowIndex == leftRow) {
                            var cornerx =
                                from +
                                (rtlmode ? -ufoCatcherMod : ufoCatcherMod) *
                                    arcSlant;
                            ...
                            if (smoothArcCurves) {
                                ...
                                if (rtlmode) {
                                    controlx = ufoCatcher
                                        ? cornerx -
                                          2 * ufoCatcherMod * reverseArcControlx
                                        : smoothArcSteepness * from +
                                          (1 - smoothArcSteepness) * cornerx;
                                    endy =
                                        leftBox.y +
                        ...
                        if (rightBox && rowIndex == rightRow) {
                            var cornerx =
                                to -
                                (rtlmode ? -1 : 1) * ufoCatcherMod * arcSlant;
                            ...
                            if (smoothArcCurves) {
                                ...
                                path.line(cornerx, -height).curveQ(
                                    controlx,
                                    -height,
                                    to,
                                    endy
            ...
            $.each(data.spans, function (spanNo, span) {
                ...
                for (
                    var connectorNo = 0;
                    connectorNo < numConnectors;
                    connectorNo++
                ) {
                    var left = span.fragments[connectorNo];
                    var right = span.fragments[connectorNo + 1];

                    if (!left.chunk.outOfPage) var leftBox = rowBBox(left);
                    if (!right.chunk.outOfPage) var rightBox = rowBBox(right);
                    var leftRow = left.chunk.row ? left.chunk.row.index : 0;
                    var rightRow = right.chunk.row
                        ? right.chunk.row.index
                        : rows.length - 1;
            ...
            $.each(rows, function (rowId, row) {
                ...
                var rowBoxHeight = Math.max(
                    row.maxArcHeight + 5,
                    row.maxSpanHeight + 1.5
                ...
                if (row.sentence) {
                    ...
                    if (rtlmode) {
                        ...
                        text = svg.text(
                            link,
                            sentNumMargin - Configuration.visual.margin.x,
                            y - rowPadding,
                            "" + row.sentence,
                            { "data-sent": row.sentence }
                        );
            ...
            var lrChunkComp = function (a, b) {
                ...
                return startDiff != 0
                    ? startDiff
                    : Util.cmp(bc.to - bc.from, ac.to - ac.from);
            };
            ...
            $.each(data.chunks, function (chunkNo, chunk) {
                ...
                if (rtlmode) {
                    ...
                    // rendering as a SVG span (as dlmat does), then the browser changes the layout on the
                    // X-axis as it likes in RTL mode.
                    svg.text(
                        textGroup,
                        chunk.textX,
                        chunk.row.textY,
                        chunk.text + nextSpace,
                        {
                ...
                if (chunk.fragments.length) {
                    ...
                    for (var i = 0; i < orderedIdx.length; i++) {
                        ...
                        c.nestingHeight =
                            c.nestingHeightLR > c.nestingHeightRL
                                ...
                                : c.nestingHeightRL;
                        c.nestingDepth =
                            c.nestingDepthLR > c.nestingDepthRL
                                ? c.nestingDepthLR
                                : c.nestingDepthRL;
                    }
                    ...
                    for (var i = 0; i < chunk.fragments.length; i++) {
                        ...
                        if (
                            ...
                            fragment.nestingHeight == 0
                        ) {
                            shrink = 1;
                        } else if (
                            fragment.nestingDepth == 0 &&
                            fragment.nestingHeight > 0
                        ) {
                            shrink = -1;
                        }
                        let entity_name =
                            fragment.span.type.indexOf("_") === -1
                                ? fragment.span.type
                                : fragment.span.type[
                                      fragment.span.type.indexOf("_") + 1
                                  ];
                        ...
                        // bit lighter
                        var lightBgColor = Util.adjustColorLightness(
                            bgColor,
                            0.8
                        );
            ...
            $.each(textMarkedRows, function (textRowNo, textRowDesc) {
                // row, from, to
                var textHighlight = svg.rect(
                    highlightGroup,
                    textRowDesc[1] - 2,
                    textRowDesc[0].textY - sizes.fragments.height,
                    textRowDesc[2] - textRowDesc[1] + 4,
                    sizes.fragments.height + 4,
        ...
        var triggerRender = function () {
            if (
                svg &&
                ((isRenderRequested && isCollectionLoaded) || requestedData) &&
                Util.areFontsLoaded()
            ) {
        ...
        var onMouseOver = function (evt) {
            ...
            if ((id = target.attr("data-span-id"))) {
                ...
                if (span.type.indexOf("_") !== -1)
                    spanDesc =
                        spanTypes[span.type.slice(span.type.indexOf("_") + 1)];
                ...
                if (that.arcDragOrigin) {
                    ...
                    var addArcAndSpan = function (arc, span) {
                        if (arc.equiv) {
                            equivs[
                                arc.eventDescId.substr(
                                    0,
                                    arc.eventDescId.indexOf("*", 2) + 1
                                )
                            ] = true;
                            var eventDesc = data.eventDescs[arc.eventDescId];
                            $.each(
                                eventDesc.leftSpans.concat(
                                    eventDesc.rightSpans
                                ),
                                function (ospanId, ospan) {
                                    spans[ospan] = true;
                                }
                            );
                        } else {
        ...
        var collectionLoaded = function (response) {
            if (!response.exception) {
                ...
                eventAttributeTypes = loadAttributeTypes(
                    response.event_attribute_types
                );
                entityAttributeTypes = loadAttributeTypes(
                    response.entity_attribute_types
                );
                spanTypes = {};
                ...
                var arcBundle =
                    (response.visual_options || {}).arc_bundle || "none";
                var textDirection =
                    (response.visual_options || {}).text_direction || "ltr";
                collapseArcs = arcBundle == "all";
                hideMidArcs = arcBundle == "hide";
                collapseArcSpace = arcBundle != "none";
                rtlmode = textDirection == "rtl";

                dispatcher.post("spanAndAttributeTypesLoaded", [
                    spanTypes,
