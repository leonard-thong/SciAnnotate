...
# Minimal standalone dlmat server based on SimpleHTTPRequestHandler.
...
class dlmatHTTPRequestHandler(SimpleHTTPRequestHandler):
    """Minimal handler for dlmat server."""
    ...
    def run_dlmat_direct(self):
        """Execute dlmat server directly."""
    ...
    def do_POST(self):
        ...
        Only implemented for dlmat server.
        """

        if self.is_dlmat():
            self.run_dlmat_direct()
        else:
            self.send_error(501, "Can only POST to dlmat")
...
class dlmatServer(ForkingMixIn, HTTPServer):
    def __init__(self, server_address):
        HTTPServer.__init__(self, server_address, dlmatHTTPRequestHandler)
...
def main(argv):
    ...
    try:
        if os.getuid() == 0:
            print("""
! WARNING: running as root. The dlmat standalone server is experimental   !
! and may be a security risk. It is recommend to run the standalone      !
! server as a non-root user with write permissions to the dlmat work/ and !
! data/ directories (e.g. apache if dlmat is set up using standard        !
! installation).                                                         !
""", file=sys.stderr)
    except AttributeError:
        # not on UNIX
        print("""
Warning: could not determine user. Note that the dlmat standalone
...
""", file=sys.stderr)
    ...
    try:
        server = dlmatServer((_DEFAULT_SERVER_ADDR, port))
        print("Serving dlmat at http://%s:%d" % server.server_address, file=sys.stderr)
