...
# Install script for dlmat server
#
...
if [ "$QUICK" = true ]; then
    ...
    while true; do
	echo 'Please enter the user name that you want to use when logging into dlmat:'
	read user_name
	if [ -n "$user_name" ]; then
	    break
	fi
    ...
    while true; do
	echo "Please enter a dlmat password (this shows on screen):"
	read password
	if [ -n "$password" ]; then
	    break
	fi
...
if [ "$QUICK" = true ]; then
    cat << EOF
Quick install: finished. To test dlmat, run the standalone server as
...
1.) If you are installing dlmat on a webserver, make sure you have 
    followed the steps described in the dlmat manual to enable CGI:

    http://dlmat.nlplab.org/installation.html

2.) Please verify that dlmat is running by accessing your installation
    using a web browser.

You can automatically diagnose some common installation issues using:

    tools/troubleshooting.sh URL_TO_dlmat_INSTALLATION

If there are issues not detected by the above script, please contact the
dlmat developers and/or file a bug to the dlmat bug tracker:

    https://github.com/nlplab/dlmat/issues'

3.) Once dlmat is running, put your data in the data directory. Or use
